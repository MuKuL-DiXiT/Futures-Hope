...
const getCommunityDatabase = async (req, res) => {
  try {
...
const createCommunity = async (req, res) => {
  try {
    ...
    if (doesExists) return res.status(400).json({ error: 'Community with this name already exists' });
...
const joinRequest = async (req, res) => {
  try {
    ...
    if (community.members.map((m) => m.toString()).includes(userId)) {
      await Community.findByIdAndUpdate(communityId, { $pull: { members: userId } }, { new: true });
    }
    const io = req.io;
    ...
    const joinRequest = await Join.create({ user: userId, community: communityId, status: 'pending' });
    ...
    io.to(`user_${community.creator}`).emit('notify', {
      from: { _id: userId, firstname: requester.firstname, lastname: requester.lastname, profilePic: requester.profilePic },
...
const joinStatus = async (req, res) => {
  try {
    ...
    const community = await Community.findById(req.params.id);
    if (community.members.map((m) => m.toString()).includes(userId)) return res.status(200).json({ status: 'joined' });
    const join = await Join.findOne({ user: req.user._id, community: req.params.id });
    if (!join) return res.status(200).json({ status: 'none' });
...
const allRequests = async (req, res) => {
  try {
    ...
    const pendingRequests = await Join.find({ community: { $in: communityIds }, status: 'pending' }).populate('user', 'firstname lastname profilePic');
...
const getCommunity = async (req, res) => {
  try {
    const community = await Community.findById(req.params.id)
      ...
      .populate({
        path: 'chatRoom',
        populate: { path: 'participants lastMessage', select: 'username firstname lastname profilePic content' },
      });
...
const acceptJoin = async (req, res) => {
  try {
    ...
    if (!joinReq || joinReq.status !== 'pending') return res.status(404).json({ message: 'Join request not found or already handled' });

    if (joinReq.community.creator.toString() !== req.user._id.toString()) return res.status(403).json({ message: 'Not authorized' });
    ...
    await Notification.create({ user: joinReq.user._id, type: 'join', from: req.user._id, message: `Your request to join ${joinReq.community.name} was accepted.` });
    const user = await User.findById(req.user._id);
    req.io.to(`user_${joinReq.user._id}`).emit('notify', {
      from: { _id: user._id, firstname: user.firstname, lastname: user.lastname },
...
const getAllCommunities = async (req, res) => {
  try {
    ...
    const communities = await Community.find({ $or: [{ members: userId }, { moderators: userId }] })
      .populate('creator', 'username profilePic')
      .populate({ path: 'chatRoom', populate: { path: 'lastMessage', populate: { path: 'sender', select: 'username profilePic' } } })
      .sort({ updatedAt: -1 });
...
const getAllUsers = async (req, res) => {
  try {
    const community = await Community.findById(req.params.comId).populate('members', 'firstname lastname email profilePic _id');
    if (!community) return res.status(404).json({ message: 'Community not found' });
    res.status(200).json({ members: community.members });
  } catch (err) {
    res.status(500).send('There is an error fetching the data');
  }
};

module.exports = {
  getCommunityDatabase,
  createCommunity,
  joinRequest,
  joinStatus,
  allRequests,
