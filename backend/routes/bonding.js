...
// Send bond request
router.post('/toggle/:receiver', verifyAccessToken, async (req, res, next) => {
  try {
    const io = req.io;
    ...
    if (existingBond) {
      if (
        ...
        existingBond.requester.toString() === currUser
      ) {
        ...
        notifyAboutBond(io, existingBond.requester, 'accepted your bond request', currentUser, "bondAccepted");
        return res.status(200).json({ message: 'Bond request accepted', bond: existingBond });
      }
    }
    notifyAboutBond(io, receiver, 'sent you a bond request', currentUser, "bondRequest");
...
router.get('/status/:userId', verifyAccessToken, async (req, res, next) => {
  try {
    const currUser = req.user._id;
...
// Get bonds for a user (accepted ones)
router.get('/allBondsAndCommunities', verifyAccessToken, async (req, res, next) => {
  try {
    ...
    const communities = await Community.find({
      members: userId
    }).select('name creator profilePic _id');
...
router.get('/pending', verifyAccessToken, async (req, res, next) => {
  try {
    const userId = req.user._id;
    ...
    const requests = await Bond.find({
      status: 'pending',
      receiver: userId
    }).populate('requester', 'profilePic firstname lastname _id');
