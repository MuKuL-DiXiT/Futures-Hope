...
router.post('/', verifyAccessToken, upload.single('screenShot'), async (req, res) => {
    try {
        const io = req.io;
        ...
        const payment = new Payment({
            ...
            email,
            amount,
            proofScreenshotUrl: req.file.path,
...
router.get('/verified', verifyAccessToken, async (req, res) => {
  try {
...
router.get('/unverified', verifyAccessToken, async (req, res) => {
  try {
...
router.patch('/verify/:paymentId', verifyAccessToken, async (req, res) => {
  try {
    const io = req.io;
    ...
    // Try to find the user who made the payment by email
    ...
    if (donor) {
      ...
      io.to(`user_${donor._id}`).emit('notify', {
        _id: notification._id,
        from: {
          _id: req.user._id,
