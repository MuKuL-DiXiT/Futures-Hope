...
export default function CommunityProfile({ comId }) {
 ...
 async function secureFetch(path, options = {}) {
  ...
  const uploadScreenshot = async (e) => {
    ...
    try {
      await secureFetch(`/auth/payment`, formData, {method:"POST", body:formData});
      alert("You won't regret donating!");
      window.location.reload();
    } catch (err) {
    ...
    <div className="min-h-screen min-w-full md:px-30  flex flex-col items-center bg-transparent">
      /* Floating Decorative Elements */
        ...
        <div className="mb-12">
          <div className="bg-gradient-to-br mb-5 from-amber-100 via-green-50 to-emerald-100 rounded-3xl p-8 shadow-xl border border-amber-200 relative overflow-hidden">
            ...
            <div className="relative flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="flex-1 text-center sm:text-left">
                ...
                <p className="text-green-700 font-medium mb-4">{community.creator.email}</p>
          ...
          {(community.qrCodeUrl) && (
            <div className="mb-12">
              <div className="bg-white bg-opacity-80 backdrop-blur-md rounded-3xl p-8 shadow-2xl border border-white border-opacity-50">
                  ...
                  <div className="flex flex-col sm:flex-row items-start gap-6">
                    <div className="relative group">
                      ...
                      <img
                        ...
                        className="relative w-40 h-40 rounded-2xl shadow-lg border-4 border-white"
                      />
                    </div>
                    <div className="flex-1">
                      <p className="text-amber-800 mb-4">Scan this QR code to make a donation and help us grow our community! Upload screenshot after donating</p>
                      <div className="flex flex-wrap gap-3">
                        <span className="px-4 py-2 bg-green-100 text-green-800 rounded-full text-sm font-semibold">🌱 Plant Trees</span>
                        <span className="px-4 py-2 bg-amber-100 text-amber-800 rounded-full text-sm font-semibold">🌍 Save Environment</span>
                        <span className="px-4 py-2 bg-emerald-100 text-emerald-800 rounded-full text-sm font-semibold">💚 Community Growth</span>
                      </div>
                      <form action={uploadScreenshot} className='flex bg-green-900/40 p-3 rounded mt-2 justify-evenly'>
                        <input type="text" placeholder='Amount (INR)' value={amount} onChange={(e) => setAmount(e.target.value)} className='bg-amber-100 rounded-full text-amber-800 focus:outline-none text-center placeholder-amber-700 focus:ring-0 ring-0 border-none' />
                        <label className='bg-green-100 text-green-800  rounded-full flex p-2  focus:outline-none text-center placeholder-amber-700 focus:ring-0 ring-0 border-none'>

                          <span className="font-medium">select scr. shot</span>
                          <ImagePlay className="w-5 h-5" />
                          <input
                            type="file"
                            onChange={handleFileChange}
                        ...
                        <button type='submit' className='bg-amber-100 text-amber-800  rounded-full p-2'>
                          submit
                        </button>
                      </form>
                    </div>
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {community.moderators.map((mod, index) => (
                <div key={mod._id} className="group relative overflow-hidden bg-white bg-opacity-70 backdrop-blur-md rounded-2xl p-6 shadow-lg border border-white border-opacity-50 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                  ...
                  <div className="relative flex items-center gap-4">
                    <div className="relative">
                      <img
                        ...
                        alt={mod.firstname}
                        className="w-16 h-16 rounded-full object-cover border-3 border-green-200 shadow-md"
                      />
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-green-700 text-sm mb-2">{mod.email}</p>
                      <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-semibold">🛡️ Guardian</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="pb-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {community.members.map((member, index) => (
              <div key={member._id} className="group relative overflow-hidden bg-white bg-opacity-60 backdrop-blur-md rounded-2xl p-6 shadow-lg border border-white border-opacity-40 hover:shadow-xl transition-all duration-300 hover:-translate-y-2 text-center">
                ...
                <div className="relative">
                  <div className="relative inline-block mb-4">
                    <img
                      ...
                      className="w-20 h-20 rounded-full mx-auto object-cover border-3 border-green-200 shadow-lg group-hover:border-green-300 transition-colors"
                    />
                    <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-amber-500 rounded-full flex items-center justify-center border-2 border-white opacity-0 group-hover:opacity-100 transition-opacity">
                      <svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <h3 className="text-lg font-bold text-amber-900 mb-2 group-hover:text-green-800 transition-colors">
                    {member.firstname} {member.lastname}
                  </h3>

                  <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="px-3 py-1 bg-amber-100 text-amber-800 rounded-full text-xs font-semibold">🌿 Member</span>
                  </div>
                </div>
              </div>
            ))}
